use crate::{isa::InvalidOpcode, static_assert};
...
impl TryFrom<u8> for Opcode {
    ...
    fn try_from(opcode: u8) -> Result<Self, Self::Error> {
        Opcode::new(opcode).ok_or_else(|| InvalidOpcode::new(opcode, None))
    }
}
...
prefixed_opcodes! {
    /// An opcode value for an instruction prefixed by a [`0xFC` opcode].
    ///
    ...
    /// An opcode value for an instruction prefixed by a [`0xFE` opcode].
    ///
    /// The feature proposals that introduced these opcodes include:
    ...
    /// An opcode value for a [vector instruction], which is an instruction prefixed by the
    /// [`0xFD` opcode].
    ///
    /// The feature proposals that introduced these opcodes include:
...
static_assert::check_size!(Opcode, <= 1);
